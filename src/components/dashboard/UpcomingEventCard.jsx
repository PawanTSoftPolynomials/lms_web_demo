...
export default function UpcomingEventCard({ task, variant = "desktop" }) {
  ...
  if (variant === "mobile") {
    ...
    return (
      <div className="rounded-xl bg-card border border-border p-3 flex items-center gap-3">
        <div className="h-9 w-9 rounded-lg bg-[#141930] border border-border flex items-center justify-center shrink-0">
          <Icon size={16} className="text-foreground" />
        </div>
        <div className="min-w-0 flex-1">
          <h4 className="text-xs font-bold text-foreground truncate">{task.title}</h4>
          <p className="text-[10px] text-muted-foreground mt-0.5">{formatDueIn(task._date)}</p>
        </div>
  ...
  return (
    <div className="p-3 rounded-xl border bg-[#141930] border-border hover:border-transparent transition">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <h4 className="text-xs font-bold text-foreground truncate">{task.title}</h4>
            <p className="text-[10px] text-muted-foreground truncate">{task.courseName || task.subtitle || badge.label}</p>
          </div>
        </div>
