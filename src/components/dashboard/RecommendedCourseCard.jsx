...
export default function RecommendedCourseCard({ course }) {
  ...
  return (
    <div className="group rounded-xl border border-border bg-[#0A0D1B] p-2.5 hover:border-primary/40 transition-all flex flex-col gap-1 min-w-0">
      <Link href={`/student/courses/${course.id}`} className="block">
        <div className="flex items-start justify-between gap-1.5">
          <div className="h-6 w-6 rounded-lg bg-muted flex items-center justify-center shrink-0 text-muted-foreground">
            <BookOpen size={11} />
          </div>
          {isNew && (
            <span className="text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-primary/15 text-primary border border-primary/25 shrink-0">
              New
            </span>
          )}
        </div>
      ...
      <Link
        ...
        className="mt-2 w-full py-1.5 px-2 rounded-lg bg-primary hover:bg-orange-600 text-slate-950 text-[10px] font-black uppercase tracking-wider transition cursor-pointer text-center"
      >
        View Course
      </Link>
    </div>
