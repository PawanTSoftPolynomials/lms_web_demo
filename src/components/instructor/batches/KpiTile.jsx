  ...
  return (
    <div className="flex-1 min-w-[150px] flex items-center gap-3 rounded-2xl bg-card border border-border p-3 shadow-sm hover:border-transparent transition">
      ...
      <div className="min-w-0">
        <p className="text-muted-foreground text-[9px] font-bold uppercase tracking-wider truncate">{label}</p>
        <div className="flex items-end gap-1.5 mt-0.5">
          <p className="text-lg font-black text-foreground leading-none">{value}</p>

          {href ? (
            <Link href={href} className="text-[9px] text-primary font-bold hover:text-orange-300 truncate hidden xl:block">
              View &rarr;
            </Link>
          ) : (
