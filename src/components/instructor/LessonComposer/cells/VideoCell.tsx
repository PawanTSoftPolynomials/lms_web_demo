...
/** Renders/edits a `type: "VIDEO"` Content row — a YouTube embed or a direct file URL. */
      ...
      {mode === "edit" ? (
        <div className="space-y-3">
          ...
          <input
            ...
            placeholder="https://youtube.com/watch?v=… or a direct video file URL"
            className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
          />
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-xl border border-card-border bg-black">
          {embedUrl ? (
            ...
            <video src={content.videoUrl} controls className="absolute inset-0 h-full w-full" />
          )}
        </div>
      ) : (
...
/** The "Add Cell" creation form for a Video block — hosted inside AddCellModal. */
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="button" size="sm" onClick={handleCreate} disabled={createContent.isPending || !videoUrl}>
