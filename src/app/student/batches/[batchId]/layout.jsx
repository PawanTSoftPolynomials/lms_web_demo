    ...
    <div className="space-y-4">
      <div>
        <Link
          ...
          className="inline-flex items-center gap-1 text-xs font-bold text-primary hover:text-orange-300 transition mb-2"
        >
        ...
        {isLoading ? (
          ...
          <>
            ...
            <p className="text-xs text-muted-foreground mt-1">{dashboard?.course?.title}</p>
          </>
        )}
      </div>

      <nav className="flex items-center gap-1.5 overflow-x-auto scrollbar-none rounded-2xl border border-border bg-card p-1.5">
        {TABS.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <Link
              ...
              className={`shrink-0 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-[11px] font-bold whitespace-nowrap transition ${
                active ? "bg-primary text-slate-950" : "text-muted-foreground hover:text-foreground hover:bg-white/5"
              }`}
            >
