...
export default function BatchAnnouncementsPage() {
  ...
  if (isLoading) {
    return (
      <div className="space-y-2.5">
        {[1, 2, 3].map((n) => (
          <div key={n} className="h-20 rounded-2xl bg-slate-800/50 animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2.5">
      {announcements.map((a) => (
        <div key={a.id} className="rounded-2xl border border-[#1A1F35] bg-[#0D1021] p-4 flex items-start gap-3">
