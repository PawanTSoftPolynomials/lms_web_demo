"use client";

import { useState } from "react";
import { useParams } from "next/navigation";
import { MessageSquare, Send } from "lucide-react";

import { useBatchDashboard } from "@/hooks/queries/student/useBatches";
// Role-agnostic — students are already permitted on every /discussions route
// (see discussion.routes.js), so this existing hook is reused as-is rather
// than duplicated under a student-namespaced copy.
import { useDiscussions, useCreateDiscussion } from "@/hooks/queries/instructor/useDiscussions";
import { formatShortDate } from "@/lib/dateUtils";

export default function BatchDiscussionsPage() {
  const params = useParams();
  const { data: dashboard } = useBatchDashboard(params.batchId);
  const courseId = dashboard?.course?.id;

  const { data: discussions = [], isLoading } = useDiscussions(courseId);
  const createDiscussion = useCreateDiscussion(courseId);

  const [title, setTitle] = useState("");
  const [body, setBody] = useState("");

  const handleSubmit = (e) => {
    e.preventDefault();
    if (!title.trim() || !body.trim() || !courseId) return;
    createDiscussion.mutate(
      { courseId, title, body },
      { onSuccess: () => { setTitle(""); setBody(""); } }
    );
  };

  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="rounded-2xl border border-border bg-card p-4 space-y-2.5">
        <input
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="Discussion title"
          className="w-full bg-background border border-transparent rounded-xl px-3.5 py-2.5 text-xs text-foreground outline-none focus:border-primary transition"
        />
        <textarea
          value={body}
          onChange={(e) => setBody(e.target.value)}
          placeholder="Ask a question or start a discussion for this batch's course..."
          rows={2}
          className="w-full bg-background border border-transparent rounded-xl px-3.5 py-2.5 text-xs text-foreground outline-none focus:border-primary transition resize-none"
        />
        <button
          type="submit"
          disabled={createDiscussion.isPending || !title.trim() || !body.trim()}
          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[11px] font-black text-slate-950 bg-primary hover:bg-orange-600 disabled:opacity-50 transition"
        >
          <Send size={12} /> Post
        </button>
      </form>

      {isLoading ? (
        <div className="space-y-2.5">
          {[1, 2].map((n) => (
            <div key={n} className="h-16 rounded-2xl bg-muted/50 animate-pulse" />
          ))}
        </div>
      ) : discussions.length === 0 ? (
        <div className="py-8 text-center text-xs text-muted-foreground border border-dashed border-border rounded-2xl">
          No discussions yet — be the first to start one.
        </div>
      ) : (
        <div className="space-y-2.5">
          {discussions.map((d) => (
            <div key={d.id} className="rounded-2xl border border-border bg-card p-4">
              <div className="flex items-start gap-3">
                <div className="h-9 w-9 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 flex items-center justify-center shrink-0">
                  <MessageSquare size={15} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-foreground truncate">{d.title}</p>
                  <p className="text-xs text-muted-foreground mt-1 leading-relaxed line-clamp-2">{d.body}</p>
                  <p className="text-[10px] text-muted-foreground mt-1.5">
                    {d.author?.name ? `${d.author.name} • ` : ""}
                    {formatShortDate(d.createdAt)}
                    {typeof d.replies?.length === "number" ? ` • ${d.replies.length} replies` : ""}
                  </p>
                </div>
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  );
}
