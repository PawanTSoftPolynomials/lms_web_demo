    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-16 animate-fade-in duration-300">
      ...
      {submitted ? (
        <Card className="p-8 border border-emerald-500/20 bg-emerald-500/5 text-center space-y-4 rounded-3xl">
          ...
          <h2 className="text-lg font-bold text-foreground tracking-tight">Feedback Submitted!</h2>
          ...
          <button
            onClick={() => router.back()}
            className="mt-6 px-5 py-2.5 bg-primary hover:bg-orange-600 text-slate-950 font-black uppercase text-xs tracking-widest rounded-xl transition cursor-pointer"
          >
            Return to Learning
          </button>
        </Card>
        ...
        <Card className="p-6 sm:p-8 border border-transparent bg-background/60 rounded-3xl space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block">Overall Satisfaction</label>
            ...
            {/* Course Rating */}
            <div className="space-y-3">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block">Quality Rating (1 to 5)</label>
            ...
            <div className="space-y-4 border-t border-transparent pt-5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block">Structured Evaluations</label>
            ...
            <div className="space-y-2 border-t border-transparent pt-5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-widest block">Detailed Review & Comments</label>
              <textarea
                rows={5}
                required
                value={comment}
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={submitting}
                className="w-full inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-2xl bg-primary hover:bg-orange-600 text-slate-950 font-black uppercase text-xs tracking-widest shadow-sm transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer border-0"
              >
                {submitting ? (
                  <>
