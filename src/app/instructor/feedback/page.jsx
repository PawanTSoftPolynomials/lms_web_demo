...
const selectClass =
  "w-full bg-white/[0.02] border border-border text-xs px-3 py-2.5 rounded-xl outline-none text-foreground focus:border-primary/60 transition";
...
function StatCard({ label, value }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-4">
      ...
      <p className="text-2xl font-black text-foreground mt-1.5">{value}</p>
    </div>
  );
}
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        {isLoading ? (
          ...
          <div className="space-y-3">
            {reviews.map((r) => (
              <div key={r.id} className="p-4 rounded-xl border border-border bg-white/[0.015]">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold text-foreground">{r.student?.user?.name || "Student"}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{r.course?.title} • {new Date(r.createdAt).toLocaleDateString()}</p>
                  </div>
                  <Stars rating={r.rating} />
                </div>
                {r.review && <p className="text-xs text-foreground mt-2.5 leading-relaxed">{r.review}</p>}
              </div>
            ))}
          </div>
        )}
